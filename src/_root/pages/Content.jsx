      ...
      <div className="flex min-w-12 flex-col items-center justify-between p-10 m-2 rounded-xl bg-black">
        <div className='basis-1/3 min-w-12 flex flex-col items-center justify-center'>
          ...
          <div className='py-1 flex my-4 items-center'>
            ...
            <button className="bg-green-700 hover:scale-110 duration-200 transform text-white rounded-sm m-2 px-2 pt-1 font-bold text-xl" onClick={addTask}>
            ...
            {openList?.items?.sort((t1,t2)=> t1.orderIndex - t2.orderIndex).map((item, index) =>
              <li key={item.id} className='text-white flex flex-row items-center justify-between gap-8 border-b-stone-500 border-b-2 border-opacity-30'>
                ...
                <div className='px-2 flex '>
                  <button className='bg-red-700 hover:scale-110 duration-200 text-white rounded-sm m-1 px-2 pt-1 font-bold' onClick={() => deleteTask(item.id)}>
                    <ion-icon name="trash-outline"></ion-icon>
                  </button>
                  <button className='bg-blue-700 hover:scale-110 duration-200 text-white rounded-sm m-1 px-2 pt-1 font-bold' onClick={() => moveTaskUp(item)}>
                    <ion-icon name="chevron-up-outline"></ion-icon>
                  </button>
                  <button  className='bg-blue-700 hover:scale-110 duration-200 text-white rounded-sm m-1 px-2 pt-1 font-bold flex items-center' onClick={() => moveTaskDown(item)}>
